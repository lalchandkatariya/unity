...
const useUserFormModal = ({data, setEvent_key}) => {//console.log("data in hooks", data)
...
useEffect(()=>{
  ...
  setCompanyName(data?.experienceDetails?.company || '')
    ...
    const userDispetch = ({userType, index}) => {
      setError(false)
      if(firstName==''||lastName==''||email==''||phoneNumber==''||dob==''||address==''||degree=='',college=='',startCollegeYear==''||companyName=='') {setError(true); return}
      ...
      userType ? dispatch({type:UPDATE_USER, payload:{data:data, willUpdateIndex:index}}):dispatch({type:ADD_USER, payload:data})
      setFirstName('')
      setLastName('')
      setEmail('')
      setPhoneNumber('')
  ...
  return {firstName, setFirstName, lastName, setLastName,
     ...
     degree, setDegree, college, setCollege, startCollegeYear, setStartCollegeYear, 
        ...
        setStartCompanyYear, endCompanyYear, setEndCompanyYear
    }
}
